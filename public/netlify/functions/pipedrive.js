...
// Dev: coloca tu token en Netlify > Site settings > Build & deploy > Environment > PIPEDRIVE_API_TOKEN
  ...
  try {
    ...
    if (!token) {
      return {
        ...
        body: JSON.stringify({ ok:false, error: "Missing env PIPEDRIVE_API_TOKEN" })
      };
    }
      ...
      if (entity === 'deal') {
        ...
        if (deal.person_id && (deal.person_id.value || deal.person_id.id)) {
          ...
          return {
            ...
            organizationName: person.org_name || deal.org_name || ''
          };
        }
